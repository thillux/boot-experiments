...
#include <nmmintrin.h>
#else
#error "CRC32 only implemented for x86"
#endif
...
static int gen_gptable(uint32_t signature, guid_t guid, unsigned nr)
{
	...
	pte[0].length = cpu_to_le32(end - GPT_HEADER_SECTOR + 1);
	...
	for (i = 0; i < GPT_ENTRY_MAX; ++i) {
		if (write(fd, &gpte[i], GPT_ENTRY_SIZE) != GPT_ENTRY_SIZE) {
			fputs("write failed.\n", stderr);
			goto fail;
		}
	}
